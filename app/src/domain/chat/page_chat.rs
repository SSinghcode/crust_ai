...
pub fn ChatPage() -> impl IntoView {
    ...
    // Keep params in scope so the route param is tracked (will be used for real DB queries later)
    let _chat_id = move || params.with(|p| p.get("id").map(|s| s.to_owned()).unwrap_or_default());
        ...
        <div class="flex flex-col h-full">
            ...
            <header class="flex items-center justify-between px-4 py-3 border-b bg-background shrink-0">
                ...
                <Button variant=ButtonVariant::Outline size=ButtonSize::Sm>
                    {move || match mode.get() {
                        ChatMode::Developer => "🦀 Developer mode",
                        ChatMode::General => "⊕ General mode",
                    }}
                </Button>
            </header>

            // ── Message area (empty until real messages arrive) ──────
